...
class CausalAttestationChain:
    ...
    def append(self, event_id, evidence, inference, policy, action, model_version, timestamp):
        ...
        partial_record = AttestationRecord(node_id=self.node_id, sequence=seq, event_id=event_id, timestamp=timestamp, parent_digest=parent, evidence_digest=ev_dig, inference_digest=inf_dig, policy_digest=pol_dig, action_digest=act_dig, model_version=model_version)
