...
| N2a callback metadata | Five C cases cover queued/active retention, positive progress, stale replay after actual heap reclamation, invalid tickets, and bounded cleanup; the queued-reference test failed before the minimal fix | Complete scripted ticket registry with a stable context; no real OS callback quiescence, child references, recycled context, cross-runtime identity, or thread synchronization |
