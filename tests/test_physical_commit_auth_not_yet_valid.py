...
@pytest.mark.asyncio
async def test_auth_not_yet_valid_never_reaches_adapter():
    class A:
        calls=0
    ...
    a=A()
    pre=DeviceState()
    physical=CapabilityLease(device_id="tv_integration_node_1",capabilities={"set_volume"},authorized_epoch=0,valid_from=datetime.now(timezone.utc)+timedelta(seconds=60))
    snapshot=PhysicalSnapshot(device_id="tv_integration_node_1",state=pre,epoch=0,observed_at=time.time())
    authorized=AuthorizedActionIntent(intent_id="future_auth_test",lease_id="lease_test",action="set_volume",device_id="tv_integration_node_1",operation="set_volume",target_state=DeviceState(volume=55.0),expected_pre_state=pre,authorization_digest="test-auth-digest",deadline_at=time.time()+30.0)
