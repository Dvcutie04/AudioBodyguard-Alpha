...
@pytest.mark.asyncio
async def test_duplicate_absorbed_never_creates_verification_record():
    class A:
        class Dev:
            class Id:
        ...
        async def execute_intent(self,intent,*args,**kwargs):
            self.calls+=1
            return ActuationReceipt(receipt_id="duplicate_receipt",intent_id=intent.intent_id,device_id=intent.device_id,transaction_id=intent.transaction_id,capability_digest=intent.capability_digest,status=ActuationStatus.DUPLICATE_ABSORBED)
    adapter=A()
    ...
    idata=dict(intent_id="duplicate_absorbed_test",device_id="tv_integration_node_1",operation="SET_VOLUME",parameters={"volume":55.0},issuer_id=key.key_id,policy_digest="policy",capability_lease_digest=lease.payload_digest,created_at=now,expires_at=now+30,nonce="intent_n",transaction_id="tx_duplicate_absorbed",protocol_version="1.0")
    ...
    assert result.receipt.status==ActuationStatus.DUPLICATE_ABSORBED
