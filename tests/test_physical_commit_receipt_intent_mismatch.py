...
@pytest.mark.asyncio
async def test_receipt_intent_mismatch_fails_after_single_actuation():
    class A:
        class Dev:
            class Id:
        ...
        async def execute_intent(self,intent,*args,**kwargs):
            self.calls+=1
            return ActuationReceipt(receipt_id="mismatch_receipt",intent_id="wrong_intent",device_id=intent.device_id,status=ActuationStatus.EXECUTED)
    adapter=A()
    ...
    idata=dict(intent_id="receipt_mismatch_test",device_id="tv_integration_node_1",operation="SET_VOLUME",parameters={"volume":55.0},issuer_id=key.key_id,policy_digest="policy",capability_lease_digest=lease.payload_digest,created_at=now,expires_at=now+30,nonce="intent_n",transaction_id="tx_receipt_mismatch",protocol_version="1.0")
    ...
    assert result.receipt.intent_id=="wrong_intent"
